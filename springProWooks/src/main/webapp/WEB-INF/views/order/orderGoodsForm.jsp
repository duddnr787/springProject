...
function imagePopup(type) {
	if (type == 'open') {
		// 팝업창을 연다.
		jQuery('#layer').attr('style', 'display:block');
...
<form  name="form_order">	
		...
		<tbody align=center>
			...
			<tr>
				<c:forEach var="item" items="${myOrderList }">
					...
					<td><h2>${item.goods_sales_price * 0.9 * item.order_goods_qty}원 (10% 할인된 가격)</h2></td>
					<td><h2>${item.goods_delivery_price }원</h2></td>
					<td><h2>${item.goods_sales_price * 0.01 * item.order_goods_qty}원</h2></td>
					<td>
					  <h2><fmt:formatNumber type="number" maxFractionDigits="3" value="${price}" var="commaPrice"/>${item.goods_sales_price * 0.9 * item.order_goods_qty + goods_delivery_price}원</h2>
		 ...
		 <table>
		   <tbody>
			 <tr class="dot_line">
				<td ><h2>이름</h2></td>
				<td>
				 <input  type="text" value="${orderer.member_name}" size="30" />
				</td>
			  </tr>
			  <tr class="dot_line">
				<td ><h2>핸드폰</h2></td>
				<td>
				 <input  type="text" value="${orderer.hp1}-${orderer.hp2}-${orderer.hp3}" size="30" />
				</td>
			  </tr>
			  <tr class="dot_line">
				<td ><h2>이메일</h2></td>
				<td>
				 <input  type="text" value="${orderer.email1}@${orderer.email2}" size="30" />
				</td>
			  </tr>
		   </tbody>
		</table>
	...
	<table width=80% class="list_view" style="background: rgb(166, 212, 252)">
		<tbody>
			...
			<tr cellpadding=40 align=center>
				...
				<td>
					<p id="p_totalDelivery">${goods_delivery_price }원</p> <input
					id="h_totalDelivery" type="hidden" value="${goods_delivery_price}" />
				</td>
				<td>
				<img width="25" alt="" 	src="${pageContext.request.contextPath}/resources/image/minus.jpg"></td>
				<td>
					<p id="p_totalSalesPrice">${goods_price * 0.1 *  order_goods_qty}원</p> 
					<input id="h_total_sales_price" type="hidden" value="${goods_delivery_price}" />
				</td>
				<td><img width="25" alt="" src="${pageContext.request.contextPath}/resources/image/equal.jpg"></td>
				<td>
					<p id="p_final_totalPrice">
						<font size="15">${goods_price * total_order_goods_qty + goods_delivery_price - (goods_price * 0.1 *  order_goods_qty)  }원 </font>
   ...
   <div class="clear"></div>
	...
	<div class="detail_table">
		<table>
			<tbody>
				...
				<tr id="tr_pay_phone" style="display:none">
				  <td>
	...
	<div class="box_buttons">
        <input type="submit" id="insertButton" class="button_submit" value="주문하기" onclick="fn_show_order_detail();">
